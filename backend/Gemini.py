...
def getResponse(message):
    ...
    content = "Using the given text, make a recipe for a meal. If this text that is given does not have anything to do with making recipes then say so, in addition to adding 'ThisIsFalse' to the end. Here is the given text: " + message
...
def formartResponse(message):
    message = "test"
    if "ThisIsFalse" in message:
        recipeName = message.replace("ThisIsFalse", "")
        recipeSteps, recipeNotes = ""
    else:
